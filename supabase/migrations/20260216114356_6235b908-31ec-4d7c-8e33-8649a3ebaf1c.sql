...
CREATE OR REPLACE FUNCTION get_link_graph_stats()
RETURNS jsonb
LANGUAGE sql STABLE
AS $$
  WITH edge_stats AS (
  ...
  -- Only count resolvable collection types (playlist_*, set_*) — reference_* never return children from Epic API
  collections_seen AS (
    ...
    WHERE rs.link_code_type = 'collection'
      ...
      AND (rs.link_code LIKE 'playlist_%' OR rs.link_code LIKE 'set_%')
  ),
  ...
  -- Also count reference_* separately for visibility
  reference_collections AS (
    SELECT count(DISTINCT rs.link_code)::int AS reference_count
    FROM discovery_exposure_rank_segments rs
    WHERE rs.link_code_type = 'collection'
      AND rs.end_ts IS NULL
      AND rs.link_code NOT LIKE 'playlist_%'
