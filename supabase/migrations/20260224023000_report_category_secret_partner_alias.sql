-- Alias internal partner category labels to a user-facing name in report section 15.
-- Keeps raw source data untouched in discover_report_islands.category.
...
AS $$
  WITH base AS (
    SELECT
      CASE
        WHEN LOWER(TRIM(COALESCE(category, ''))) IN ('perfectkiwi', 'perfect kiwi')
          THEN 'Fortnite Secret Partner'
        ELSE COALESCE(NULLIF(INITCAP(LOWER(category)), ''), 'Fortnite UGC')
      END AS cat_name,
