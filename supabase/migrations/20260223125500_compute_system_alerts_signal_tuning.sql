...
DECLARE
  ...
  v_last_exposure_tick_age_seconds INT := NULL;
  v_exposure_ticks_1h INT := 0;
  v_last_metadata_fetch TIMESTAMPTZ := NULL;
  v_last_metadata_fetch_age_seconds INT := NULL;
  ...
  v_exposure_stale_severity TEXT := 'ok';
  v_metadata_backlog_severity TEXT := 'ok';
  v_link_edges_coverage_severity TEXT := 'ok';
BEGIN
  ...
  WHERE COALESCE(t.last_status, 'idle') <> 'paused'
    AND (
  ...
  IF v_last_exposure_tick IS NOT NULL THEN
    v_last_exposure_tick_age_seconds := EXTRACT(epoch FROM (now() - v_last_exposure_tick))::int;
  END IF;
  ...
  IF v_last_metadata_fetch IS NOT NULL THEN
    v_last_metadata_fetch_age_seconds := EXTRACT(epoch FROM (now() - v_last_metadata_fetch))::int;
  END IF;
  ...
  -- Signal tuning: keep critical only for real outages, not partial degradation.
  v_exposure_stale_severity := CASE
    WHEN v_exposure_stale = 0 THEN 'ok'
    WHEN v_exposure_stale <= 3 AND v_exposure_ticks_1h >= 3 THEN 'warn'
    WHEN v_exposure_stale <= 1 THEN 'warn'
    ELSE 'error'
  END;

  v_metadata_backlog_severity := CASE
    WHEN v_meta_due < 500 THEN 'ok'
    WHEN v_meta_due < 5000 THEN 'warn'
    WHEN v_metadata_fetched_1h >= 50 THEN 'warn'
    ELSE 'error'
  END;

  v_link_edges_coverage_severity := CASE
    WHEN v_collections_total = 0 THEN 'ok'
    WHEN v_edges_parents = 0 AND v_collections_total <= 3 THEN 'warn'
  ...
  VALUES
    (
      'exposure_stale',
      v_exposure_stale_severity,
      CASE
        WHEN v_exposure_stale = 0 THEN 'Exposure OK'
        WHEN v_exposure_stale <= 3 AND v_exposure_ticks_1h >= 3 THEN 'Exposure partially stale targets detected'
        ELSE 'Exposure stale targets detected'
      END,
    ...
    (
      'metadata_backlog',
      v_metadata_backlog_severity,
      CASE
        WHEN v_meta_due < 500 THEN 'Metadata backlog OK'
        WHEN v_meta_due < 5000 THEN 'Metadata backlog moderate'
        WHEN v_metadata_fetched_1h >= 50 THEN 'Metadata backlog high but draining'
        ELSE 'Metadata backlog growing'
      END,
      jsonb_build_object('due_now', v_meta_due, 'fetched_1h', v_metadata_fetched_1h),
      now()
    ),
    (
      'intel_freshness',
    ...
    (
      'link_edges_coverage',
      v_link_edges_coverage_severity,
      CASE
        WHEN v_collections_total = 0 THEN 'Link edges coverage'
        WHEN v_edges_parents = 0 AND v_collections_total <= 3 THEN 'Link edges bootstrap pending'
        WHEN v_edges_parents = 0 THEN 'No link edges resolved'
        ELSE 'Link edges coverage'
      END,
      jsonb_build_object(
        'edges_total', v_edges_total,
        'parents_resolved', v_edges_parents,
        'collections_resolvable', v_collections_total,
        'collections_total', v_collections_total,
        'bootstrap', (v_edges_parents = 0 AND v_collections_total <= 3)
      ),
      now()
    ),
    (
    ...
    (
      ...
      jsonb_build_object(
        ...
        'last_tick_age_seconds', v_last_exposure_tick_age_seconds,
    ...
    (
      ...
      jsonb_build_object(
        ...
        'last_fetch_age_seconds', v_last_metadata_fetch_age_seconds,
