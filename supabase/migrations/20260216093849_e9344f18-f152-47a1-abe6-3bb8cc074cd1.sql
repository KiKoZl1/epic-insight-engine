...
-- Fix exposure analysis RPC to use inclusive end date (<=)
CREATE OR REPLACE FUNCTION public.report_finalize_exposure_analysis(
  p_report_id text,
      ...
      (
        ...
        FROM (
          ...
          WHERE rd.link_code = ro.link_code
            AND rd.date >= v_week_start
            AND rd.date <= v_week_end
...
-- Fix exposure efficiency RPC to use inclusive end date (<=)
