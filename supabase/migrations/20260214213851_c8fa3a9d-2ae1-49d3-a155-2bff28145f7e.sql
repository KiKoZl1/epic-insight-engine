...
BEGIN
  ...
  counts AS (
    SELECT COUNT(*)::int AS entries_count, COUNT(DISTINCT panel_name)::int AS panels_count
    FROM incoming
  )
  ...
  WITH incoming_presence_keys AS (
    SELECT DISTINCT panel_name, link_code FROM incoming
  )
  ...
  WITH incoming_rank_keys AS (
    ...
    FROM incoming GROUP BY surface_name, panel_name, rank, link_code
  )
  ...
  -- Close absent ranks
  WITH incoming_rank_keys AS (
    SELECT DISTINCT panel_name, rank FROM incoming
  )
  UPDATE public.discovery_exposure_rank_segments s
  SET end_ts = s.last_seen_ts, closed_reason = 'absent_confirmed'
  WHERE s.target_id = p_target_id AND s.end_ts IS NULL
