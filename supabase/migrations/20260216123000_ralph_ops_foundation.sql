-- Ralph operations foundation (runs, actions, evals, incidents)
...
CREATE INDEX IF NOT EXISTS ralph_runs_status_started_idx
  ON public.ralph_runs (status, started_at DESC);

CREATE INDEX IF NOT EXISTS ralph_runs_mode_started_idx
  ON public.ralph_runs (mode, started_at DESC);

CREATE INDEX IF NOT EXISTS ralph_actions_run_step_idx
  ON public.ralph_actions (run_id, step_index);

CREATE INDEX IF NOT EXISTS ralph_eval_run_suite_idx
  ON public.ralph_eval_results (run_id, suite, created_at DESC);

CREATE INDEX IF NOT EXISTS ralph_incidents_open_idx
  ON public.ralph_incidents (resolved, severity, created_at DESC);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='ralph_runs' AND policyname='select_ralph_runs_admin_editor'
  ) THEN
    CREATE POLICY select_ralph_runs_admin_editor
      ON public.ralph_runs
      FOR SELECT
      TO authenticated
      USING (
        EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_actions' AND policyname='select_ralph_actions_admin_editor'
  ) THEN
    CREATE POLICY select_ralph_actions_admin_editor
      ON public.ralph_actions
      FOR SELECT
      TO authenticated
      USING (
        EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_eval_results' AND policyname='select_ralph_eval_admin_editor'
  ) THEN
    CREATE POLICY select_ralph_eval_admin_editor
      ON public.ralph_eval_results
      FOR SELECT
      TO authenticated
      USING (
        EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_incidents' AND policyname='select_ralph_incidents_admin_editor'
  ) THEN
    CREATE POLICY select_ralph_incidents_admin_editor
      ON public.ralph_incidents
      FOR SELECT
      TO authenticated
      USING (
        EXISTS (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_runs' AND policyname='all_ralph_runs_service_role'
  ) THEN
    CREATE POLICY all_ralph_runs_service_role
      ON public.ralph_runs
      FOR ALL
      TO service_role
      USING (true)
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_actions' AND policyname='all_ralph_actions_service_role'
  ) THEN
    CREATE POLICY all_ralph_actions_service_role
      ON public.ralph_actions
      FOR ALL
      TO service_role
      USING (true)
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_eval_results' AND policyname='all_ralph_eval_service_role'
  ) THEN
    CREATE POLICY all_ralph_eval_service_role
      ON public.ralph_eval_results
      FOR ALL
      TO service_role
      USING (true)
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname='public' AND tablename='ralph_incidents' AND policyname='all_ralph_incidents_service_role'
  ) THEN
    CREATE POLICY all_ralph_incidents_service_role
      ON public.ralph_incidents
      FOR ALL
      TO service_role
      USING (true)
...
CREATE OR REPLACE FUNCTION public.start_ralph_run(
  ...
  p_target_scope text[] DEFAULT '{}',
  p_max_iterations integer DEFAULT 8,
  p_timeout_minutes integer DEFAULT 45,
...
BEGIN
  ...
  VALUES (
    ...
    'running',
    GREATEST(COALESCE(p_max_iterations, 8), 1),
    GREATEST(COALESCE(p_timeout_minutes, 45), 1),
    GREATEST(COALESCE(p_budget_usd, 0), 0),
    GREATEST(COALESCE(p_token_budget, 0), 0),
    COALESCE(p_target_scope, '{}'),
...
CREATE OR REPLACE FUNCTION public.finish_ralph_run(
  ...
  p_spent_tokens bigint DEFAULT NULL,
...
BEGIN
  v_status := CASE
    WHEN p_status IN ('running','completed','failed','cancelled','promotable','rolled_back') THEN p_status
    ELSE 'failed'
  END;

  UPDATE public.ralph_runs
  SET
    status = v_status,
    ended_at = CASE WHEN v_status = 'running' THEN ended_at ELSE now() END,
    summary = CASE
      WHEN p_summary IS NULL THEN summary
      ELSE COALESCE(summary, '{}'::jsonb) || p_summary
    END,
    error_message = COALESCE(p_error_message, error_message),
    spent_tokens = COALESCE(p_spent_tokens, spent_tokens),
    spent_usd = COALESCE(p_spent_usd, spent_usd),
    updated_at = now()
  WHERE id = p_run_id;
...
BEGIN
  ...
  INSERT INTO public.ralph_actions (
    run_id,
    step_index,
    phase,
    tool_name,
...
DECLARE
  v_id bigint;
BEGIN
  INSERT INTO public.ralph_eval_results (
    run_id,
    suite,
    metric,
    value,
    ...
    p_run_id,
    COALESCE(NULLIF(p_suite, ''), 'default'),
    COALESCE(NULLIF(p_metric, ''), 'unknown_metric'),
    p_value,
    p_threshold,
...
CREATE OR REPLACE FUNCTION public.raise_ralph_incident(
  ...
  p_incident_type text DEFAULT 'generic',
  p_message text DEFAULT 'incident',
...
BEGIN
  ...
  INSERT INTO public.ralph_incidents (
    run_id,
    severity,
    incident_type,
    message,
    ...
    p_run_id,
    v_severity,
    COALESCE(NULLIF(p_incident_type, ''), 'generic'),
    COALESCE(NULLIF(p_message, ''), 'incident'),
...
CREATE OR REPLACE FUNCTION public.resolve_ralph_incident(
...
AS $$
  WITH w AS (
    SELECT *
    FROM public.ralph_runs
    WHERE started_at >= now() - make_interval(hours => GREATEST(COALESCE(p_hours, 24), 1))
  ),
  durations AS (
    SELECT EXTRACT(EPOCH FROM (ended_at - started_at)) * 1000 AS ms
    FROM w
  ...
  SELECT jsonb_build_object(
    ...
    'success_rate_pct', (
      SELECT CASE WHEN COUNT(*) > 0
        THEN ROUND((COUNT(*) FILTER (WHERE status IN ('completed','promotable'))::numeric * 100.0 / COUNT(*)), 2)
        ELSE 0 END
      FROM w
    ),
    'avg_duration_ms', (
      SELECT ROUND(AVG(ms))::int FROM durations
    ),
    'p95_duration_ms', (
      SELECT ROUND(percentile_cont(0.95) WITHIN GROUP (ORDER BY ms))::int FROM durations
    ),
    'open_incidents', (
      SELECT COUNT(*)::int FROM public.ralph_incidents WHERE resolved = false
    ),
    'critical_open_incidents', (
      SELECT COUNT(*)::int FROM public.ralph_incidents WHERE resolved = false AND severity = 'critical'
    ),
    'last_run_at', (
      SELECT MAX(started_at) FROM public.ralph_runs
    )
  );
$$;
