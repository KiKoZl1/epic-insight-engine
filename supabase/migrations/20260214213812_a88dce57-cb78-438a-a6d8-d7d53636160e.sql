...
BEGIN
  ...
  WITH segs AS (
    SELECT
      s.target_id, s.surface_name, s.panel_name, s.link_code, s.link_code_type,
      s.rank, s.ccu_max,
  ...
  overlapped AS (
    SELECT target_id, surface_name, panel_name, link_code, link_code_type, rank, ccu_max,
      EXTRACT(EPOCH FROM (b - a))::double precision AS secs
    FROM segs WHERE b > a
  ),
  agg AS (
    SELECT
      p_date AS date, target_id, surface_name, panel_name, link_code, link_code_type,
  ...
  INSERT INTO public.discovery_exposure_rollup_daily (
    ...
    minutes_exposed, appearances, best_rank, avg_rank, ccu_max_seen, distinct_creators
  )
  SELECT date, target_id, surface_name, panel_name, link_code, link_code_type,
    minutes_exposed, appearances, best_rank, avg_rank, ccu_max_seen,
    CASE WHEN link_code_type = 'island' THEN 1 ELSE NULL END
  FROM agg
  ON CONFLICT (date, target_id, panel_name, link_code)
  DO UPDATE SET
    surface_name = EXCLUDED.surface_name, link_code_type = EXCLUDED.link_code_type,
    minutes_exposed = EXCLUDED.minutes_exposed, appearances = EXCLUDED.appearances,
    best_rank = EXCLUDED.best_rank, avg_rank = EXCLUDED.avg_rank,
    ccu_max_seen = EXCLUDED.ccu_max_seen, distinct_creators = EXCLUDED.distinct_creators;
...
AS $$
  WITH base AS (
    SELECT r.target_id, MAX(r.surface_name) AS surface_name, r.panel_name, r.link_code,
      MAX(r.link_code_type) AS link_code_type,
      SUM(r.minutes_exposed)::int AS minutes_exposed, MAX(r.ccu_max_seen)::int AS ccu_max_seen,
  ...
  ranked AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY target_id, panel_name ORDER BY minutes_exposed DESC, ccu_max_seen DESC NULLS LAST, link_code ASC) AS rn
    FROM base
  )
  SELECT target_id, surface_name, panel_name, link_code, link_code_type,
    minutes_exposed, ccu_max_seen, best_rank, avg_rank
  FROM ranked WHERE rn <= GREATEST(p_limit_per_panel, 1)
...
AS $$
  ...
  islands AS (
    ...
    FROM base b WHERE b.link_code_type = 'island'
  ),
  coll AS (
    SELECT b.date, b.target_id, b.surface_name, b.panel_name, b.link_code
    FROM base b WHERE b.link_code_type = 'collection'
  ),
  creators AS (
    SELECT DISTINCT i.date, i.target_id, i.surface_name, i.panel_name, c.creator_code
    FROM islands i
    JOIN public.discover_islands_cache c ON c.island_code = i.link_code
  ...
  LEFT JOIN (
    ...
    FROM islands i2 GROUP BY i2.date, i2.target_id, i2.surface_name, i2.panel_name
  ...
  LEFT JOIN (
    ...
    FROM creators cr2 GROUP BY cr2.date, cr2.target_id, cr2.surface_name, cr2.panel_name
  ...
  LEFT JOIN (
    ...
    FROM coll col2 GROUP BY col2.date, col2.target_id, col2.surface_name, col2.panel_name
